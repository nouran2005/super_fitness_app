  ...
  group('WeightSection', () {
    testWidgets('renders titles and wheel picker label correctly', (
      ...
      await tester.runAsync(() async {
        await tester.pumpWidget(buildTestWidget(child: const WeightSection()));
      });
      await tester.pumpAndSettle();

      expect(find.text('whatIsYourWeight'), findsOneWidget);
      expect(find.text('personalizedPlanMsg'), findsOneWidget);
      expect(find.text('kg'), findsOneWidget);
    });

    testWidgets('Next button is disabled when weight is null', (tester) async {
      await tester.runAsync(() async {
        await tester.pumpWidget(buildTestWidget(child: const WeightSection()));
      });
      await tester.pumpAndSettle();
    ...
    testWidgets('Next button is enabled when weight is set', (tester) async {
      final cubit = seededCubit(
        SignupStates(currentStep: SignupStep.weight, weight: 70),
      );
      await tester.runAsync(() async {
        await tester.pumpWidget(
          buildTestWidget(child: const WeightSection(), cubit: cubit),
        );
      });
      await tester.pumpAndSettle();
    ...
    testWidgets('tapping Next when weight is set advances to height step', (
      tester,
    ) async {
      final cubit = seededCubit(
        SignupStates(currentStep: SignupStep.weight, weight: 70),
      ...
      expect(cubit.state.currentStep, SignupStep.height);
    });
  });
}
