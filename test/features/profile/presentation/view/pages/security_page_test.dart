...
void main() {
    ...
    return Builder(
      builder: (context) {
        return BlocProvider<ProfileCubit>.value(
          value: profileCubit,
          child: MaterialApp(locale: const Locale('en'), home: SecurityPage()),
        );
      },
    );
  }

  testWidgets('renders SecurityPage loading state', (tester) async {
    when(
      profileCubit.state,
    ).thenReturn(ProfileState(security: Resource.loading()));
  ...
  testWidgets('renders SecurityPage error state', (tester) async {
    when(profileCubit.state).thenReturn(
      ProfileState(security: Resource.error('Failed to load security data')),
    );

    await tester.pumpWidget(buildTest());

    expect(find.text('Failed to load security data'), findsOneWidget);
  });
